...
e irá retornar um vetor com o número de vezes que ele bateu seu recorde de 
maior número de pontos e quando fez seu pior jogo. (Número do pior jogo).
Obs.: O primeiro jogo não conta como novo recorde do melhor.
Exemplo:
String: “10 20 20 8 25 3 0 30 1”
Retorno: [3, 7] (Significa que ele bateu três vezes seu recorde de melhor pontuação e 
a pior pontuação aconteceu no sétimo jogo.)*/

function Pontuacao(pont = "10 20 20 8 25 3 0 30 1"){
    let res = pont.split(" "), record = res[0], pior = 0, nvezes = 0, jogo = 0

    for (i in res){
        if (Number(res[i]) > Number(record)){
            nvezes++
            record = res[i]
        }
        if (Number(res[i]) <= Number(res[pior])){
            pior = i
        }
    }
    return [nvezes, Number(pior)+1]
}
